...
Telegram Bot と Next.js Web UI で動く個人向けナレッジ管理ツール。メモや URL を送ると AI が自動で要約・タグ付け・記憶して、あとから自然言語で検索できる。
...
- **Web UI** — ブラウザからナレッジベースを検索・閲覧・管理（Next.js）
...
Web UI は Telegram Bot と同じ SQLite データベースを共有します。両方を同時に実行できます。

## Web UI

Next.js Web UI でブラウザからナレッジベースにアクセスできます:

- **検索** — FTS5 による全文検索（Bot と同じエンジン）
- **アイテム詳細** — コンテンツ全文、要約、タグ、OG画像の表示。削除も可能
- **タグ** — 全タグの一覧とタグ別フィルタリング
- **インサイト** — AI が最近の関心や傾向をまとめた週間ダイジェスト、忘れかけた知識の再発見、重要メモリ、カテゴリブラウザ
- **統計** — アイテム数、タグ数、メモリ統計、タイプ別分布のダッシュボード
- **設定** — UI 言語の切り替え（英語、日本語、中国語簡体/繁体、スペイン語、ブラジルポルトガル語、韓国語）
...
│   ├── page.tsx              # 検索（メイン）
│   ├── items/[id]/page.tsx   # アイテム詳細
│   ├── tags/page.tsx         # 全タグ一覧
│   ├── tags/[name]/page.tsx  # タグ別アイテム
│   ├── insight/page.tsx      # メモリインサイト + AI ダイジェスト
│   ├── stats/page.tsx        # 統計ダッシュボード
│   └── settings/page.tsx     # 言語設定
├── lib/
│   ├── db.ts                 # DB クエリ（共有 SQLite）
│   ├── actions.ts            # Server Actions
│   ├── i18n.ts               # 国際化（7言語対応）
│   └── ai.ts                 # LLM インサイトサマリー生成
